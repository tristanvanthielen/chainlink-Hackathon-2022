...
def create_subscription():
    if not config["networks"][network.show_active()].get("subscription_id", False):
        account = get_account()
        ...
        if network.show_active() not in NON_FORKED_LOCAL_BLOCKCHAIN_ENVIRONMENTS:
            with open(config_path, "r") as file:
                ...
                configuration["networks"][network.show_active()]["subscription_id"] = int(
                    subscription_id
                )
...
def fund_subscription(subscription_id=1):
    ...
    if network.show_active() not in NON_FORKED_LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        ...
        vrf_coordinator = get_contract("vrf_coordinator")
        tx = vrf_coordinator.fundSubscription(subscription_id, fund_amount, {"from": account})
        tx.wait(1)
